...
public class TemperatureProcessor {
    ...
    @PreDestroy
    void onStop() {
        LOG.info("Stream Aggregator listener is closing...");
    ...
    private Cancellable createStreamListener() {
        ...
        return Multi.createBy()
                ...
                .transformToUniAndMerge(entry ->
                        this.hash.hget(AGGREGATE_TABLE, entry.stationId.toString())
                                .map(oldAgg -> {
                ...
                .onFailure().invoke(err -> LOG.errorv("Caught exception: {0}", err))
                .call(agg -> {
                    LOG.infov("Acknowledge ids: {0}", agg.messageIds.toString() );
                    return this.stream.xack(TEMPERATURE_VALUES_STREAM, CONSUMER_GROUP,
                            agg.messageIds.toArray(String[]::new));
                })
                .onFailure().invoke(err -> LOG.errorv("Caught exception: {0}", err))
                .subscribe()
    ...
    private TemperatureAggregate setWeatherStationName(TemperatureAggregate agg, String response) {
        ...
        LOG.debugv("Retrieved Station: {0}", response);
        if (isJson(response)) {
