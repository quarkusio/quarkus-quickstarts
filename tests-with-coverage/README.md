...
The solution is located in the `tests-with-coverage` directory.
...
The project will also need some tests. First a simple JUnit ```GreetingServiceTest.java```:
...
But also a @QuarkusTest ```GreetingResourceTest.java```:
...
One will be our example of a Unit Test and one will be our example of Integration Test.
...
If you run ```mvn clean test```, the coverage information will be collected during the execution of the unit tests in the file ```jacoco.exec```.
...
During the execution of the generation of the report, you may have noticed a warning:
